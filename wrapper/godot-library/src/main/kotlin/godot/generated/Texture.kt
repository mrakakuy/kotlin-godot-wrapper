...
open class Texture : Resource {
    ...
    open fun draw(canvasItem: RID, position: Vector2, modulate: Color = Color(1,1,1,1), transpose: Boolean = false, normalMap: Texture? = null) {
        _icall_Unit_RID_Vector2_Color_Boolean_nObject(drawMethodBind, this.rawMemory, canvasItem, position, modulate, transpose, normalMap)
    }


    private val drawRectMethodBind: CPointer<godot_method_bind> by lazy { getMB("Texture", "draw_rect") }
    open fun drawRect(canvasItem: RID, rect: Rect2, tile: Boolean, modulate: Color = Color(1,1,1,1), transpose: Boolean = false, normalMap: Texture? = null) {
        _icall_Unit_RID_Rect2_Boolean_Color_Boolean_nObject(drawRectMethodBind, this.rawMemory, canvasItem, rect, tile, modulate, transpose, normalMap)
    }


    private val drawRectRegionMethodBind: CPointer<godot_method_bind> by lazy { getMB("Texture", "draw_rect_region") }
    open fun drawRectRegion(canvasItem: RID, rect: Rect2, srcRect: Rect2, modulate: Color = Color(1,1,1,1), transpose: Boolean = false, normalMap: Texture? = null, clipUv: Boolean = true) {
        _icall_Unit_RID_Rect2_Rect2_Color_Boolean_nObject_Boolean(drawRectRegionMethodBind, this.rawMemory, canvasItem, rect, srcRect, modulate, transpose, normalMap, clipUv)
    }
